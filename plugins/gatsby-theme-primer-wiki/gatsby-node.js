        ...
        return {
          ...
          items: item.nodes.map((child) => {
            return {
              ...
              lastUpdated: child.fields.lastUpdated,
              body: child.body,
            };
          }),
        };
...
exports.onCreateNode = async (
  ...
  if (node.internal.type === `Mdx`) {
    ...
    const blameUrl = getBlameUrl(options.blameUrl, parentNode.relativePath);
...
function getBlameUrl(baseBlameUrl, filePath) {
  if (baseBlameUrl && filePath) {
    return `${baseBlameUrl}${filePath}`;
  } else {
    return "";
  }
}
